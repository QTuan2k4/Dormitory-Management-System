...
//    }
	...
	public String listStudents(@RequestParam(required = false) String search,
			@RequestParam(required = false) String status, @RequestParam(required = false) String specialCircumstance,
		...
		if (status != null && !status.isBlank()) {
			try {
				...
				students = students.stream().filter(s -> s.getRegistrationStatus() == regStatus).toList();
		...
		if (search != null && !search.isBlank()) {
			...
			students = students.stream().filter(s -> {
				...
				return fullName.contains(keyword) || studentId.contains(keyword) || phone.contains(keyword);
			}).toList();
		}
	...
	// Alias mapping for legacy URL '/admin/student-list' -> redirect to
	// '/admin/students'
	...
	public String rejectRegistration(@PathVariable int id, @RequestParam("rejectionReason") String rejectionReason, // THÊM
																													// tham
																													// số
																													// lý
																													// do
	...
	public String showAssignRoomForm(@PathVariable int id, @RequestParam(required = false) Integer buildingId,
			Model model) {
		...
		try {
			...
			if (floorInt < 1 || floorInt > 6)
				floorInt = 6;
		} catch (Exception e) {
			floorInt = 6;
		}

		Map<Integer, Long> feeMap = Map.of(1, 3_000_000L, 2, 2_700_000L, 3, 2_400_000L, 4, 2_100_000L, 5, 1_800_000L, 6,
				1_500_000L);

		if (buildingId != null) {
			final int targetFloor = floorInt;
			availableRooms = roomService.findRoomsByBuildingId(buildingId).stream()
					.filter(r -> r.getFloor() == targetFloor).filter(r -> r.getCurrentOccupants() < r.getSlot())
					.sorted(Comparator.comparing(Rooms::getFloor).thenComparing(Rooms::getRoomNumber)).toList();
		}
		Buildings selectedBuilding = null;
		if (buildingId != null) {
			selectedBuilding = buildings.stream().filter(b -> b.getId() == buildingId).findFirst().orElse(null);
		}
	...
	public String assignRoom(@PathVariable int id, @RequestParam int roomId, RedirectAttributes ra) {
	...
	public String viewDormitoryDetail(@PathVariable("id") int buildingId, Model model) {
		...
		int totalRooms = roomsByFloor.values().stream().mapToInt(List::size).sum();
